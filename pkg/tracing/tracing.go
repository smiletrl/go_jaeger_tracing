...
type Provider interface {
	// set up tracer
	SetupTracer(serviceName, endpoint, stage string) (io.Closer, error)
...
func (p *provider) SetupTracer(serviceName, endpoint, stage string) (io.Closer, error) {
	...
	closer, err := cfg.InitGlobalTracer(
		fmt.Sprintf("%s.%s", serviceName, stage),
